...
func rDeclare(rconn *rConn, clientType int, channel *rChannel, exChangeName string, exChangeType string, queueName string, route string, isDeadQueue bool, oldExChangeName string, oldQueueName, oldRoute string) (*rChannel, error) {
	if clientType == RABBITMQ_TYPE_PUBLISH {
		if (len(exChangeType) == 0) || (exChangeType != EXCHANGE_TYPE_DIRECT && exChangeType != EXCHANGE_TYPE_FANOUT && exChangeType != EXCHANGE_TYPE_TOPIC && exChangeType != EXCHANGE_TYPE_DELAYED_MESSAGE) {
			return channel, errors.New("交换机类型错误")
		}
	}
	newChannel := channel.ch
	var amqpTable amqp.Table
	if exChangeType == EXCHANGE_TYPE_DELAYED_MESSAGE {
		// 死信
	...
	err := newChannel.ExchangeDeclare(exChangeName, exChangeType, true, false, false, false, amqpTable)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("MQ注册交换机失败:%s", err))
	}

	if (clientType != RABBITMQ_TYPE_PUBLISH && exChangeType != EXCHANGE_TYPE_FANOUT) || (clientType == RABBITMQ_TYPE_CONSUME && (exChangeType == EXCHANGE_TYPE_FANOUT || exChangeType == EXCHANGE_TYPE_DIRECT || exChangeType == EXCHANGE_TYPE_DELAYED_MESSAGE)) {
...
func retryConsume(pool *RabbitPool) {
	log(fmt.Sprintf("5秒后开始重试:[%d]", pool.consumeCurrentRetry))
...
func consumeTask(num int32, pool *RabbitPool, receive *ConsumeReceive) {
	...
	if len(receive.Route) > 0 {
		deadRouteKey = receive.Route //fmt.Sprintf("%s-%s", receive.Route, "dead")
	}
	...
	if receive.IsTry {
		if num%2 == 0 {
			...
			deadRChanels, err = rDeclare(conn, pool.clientType, deadRChanels, deadExchangeName, receive.ExchangeType, deadQueueName, deadRouteKey, true, receive.ExchangeName, receive.QueueName, receive.Route)
		}
	}
	if err != nil {
		if receive.EventFail != nil {
...
func rPush(pool *RabbitPool, data *RabbitMqData, sendTime int) *RabbitMqError {
	...
	if err != nil {
		...
		if data.ExchangeType == EXCHANGE_TYPE_DELAYED_MESSAGE {
			bytes, _ := json.Marshal(data.Data)
			var pub amqp.Publishing
			_ = json.Unmarshal(bytes, &pub)
			err = rChannels.ch.Publish(data.ExchangeName, data.Route, false, false, pub)
		} else {
			pub, ok := data.Data.(string)
			if !ok {
				fmt.Println("Failed to convert to string.")
				return NewRabbitMqError(RCODE_GET_CHANNEL_ERROR, "传输内容错误", err.Error())
			}
		...
		/*err = rChannels.ch.Publish(data.ExchangeName, data.Route, false, false, data.Data)*/ /*amqp.Publishing{
			ContentType:  "text/plain",
			Body:         []byte(data.Data),
			DeliveryMode: amqp.Persistent, //持久化到磁盘
		}*/
